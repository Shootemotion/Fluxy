  ...
  return (
    <div ref={wrapRef} className="relative">
      <div className="relative">
        ...
        {loading && (
          <span className="absolute right-3 top-1/2 -translate-y-1/2" style={{ color: "var(--fg-6)" }}>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold" style={{ color: "var(--fg-5)" }}>Patrimonio</h1>
          <p className="text-sm mt-0.5" style={{ color: "var(--fg-6)" }}>Activos, inversiones y pasivos</p>
        </div>
        ...
        {(["resumen","inversiones","activos","pasivos","plazos"] as Tab[]).map(t => {
          ...
          return (
            <button key={t} onClick={() => setTab(t)}
              ...
              style={tab === t ? { background: "rgba(108,99,255,0.25)", color: "#A5A0FF" } : { color: "var(--fg-6)" }}>
              {labels[t]}
            </button>
          );
        })}
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { label: "Total activos",   val: totalActivosARS, color: "var(--fg-hi)", accent: "#6C63FF" },
              { label: "Total pasivos",   val: totalPasivosARS, color: "#EF4444", accent: "#EF4444" },
              { label: "Plazos fijos",   val: totalPlazosARS, color: "var(--fg-hi)", accent: "#F59E0B" },
              { label: "TC referencia",   val: TC_REF, color: "var(--fg-hi)", accent: "#F59E0B", isTc: true },
            ].map(k => (
              <div key={k.label} className="kpi-card" style={{ "--accent": k.accent } as React.CSSProperties}>
                <p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: "var(--fg-6)" }}>{k.label}</p>
          ...
          {(totalActivosFisicos > 0 || totalInversionesARS > 0 || totalPasivosARS > 0) ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              ...
              <div className="glass-card p-6">
                <h2 className="text-base font-semibold mb-4" style={{ color: "var(--fg-hi)" }}>Composición patrimonial</h2>
              ...
              <div className="glass-card overflow-hidden">
                <div className="p-5 pb-3">
                  <h2 className="text-base font-semibold" style={{ color: "var(--fg-hi)" }}>Detalle</h2>
                </div>
                {[
            ...
            <div className="glass-card p-16 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2" style={{ color: "var(--fg-hi)" }}>Empezá a registrar tu patrimonio</h3>
              <p className="text-sm max-w-sm mx-auto" style={{ color: "var(--fg-5)" }}>
      ...
      {tab === "inversiones" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <p className="text-xs" style={{ color: "var(--fg-6)" }}>
          ...
          {posiciones.length === 0 ? (
            <div className="glass-card p-14 text-center">
              <p className="text-5xl mb-4">📈</p>
              <h3 className="text-lg font-semibold mb-2" style={{ color: "var(--fg-hi)" }}>Sin posiciones</h3>
              <p className="text-sm max-w-sm mx-auto mb-4" style={{ color: "var(--fg-5)" }}>
                Agregá tus acciones, CEDEARs o criptomonedas con su ticker de Yahoo Finance.
              </p>
              <p className="text-xs mb-6" style={{ color: "var(--fg-7)" }}>
            ...
            <div className="glass-card overflow-hidden">
                      ...
                      <tr key={pos.id}>
                        <td>
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <p className="text-sm font-semibold" style={{ color: "var(--fg-hi)" }}>{pos.ticker}</p>
                              {(pos.nombre || pos.broker) && <p className="text-xs" style={{ color: "var(--fg-6)" }}>{[pos.nombre, pos.broker].filter(Boolean).join(" · ")}</p>}
                            </div>
                          </div>
                        </td>
                        <td style={{ textAlign: "right" }}>
                          {q?.price != null
                            ? <span className="font-mono text-sm font-semibold" style={{ color: "var(--fg-2)" }}>{q.currency === "ARS" ? "$" : "U$S"} {q.price.toLocaleString("es-AR", { maximumFractionDigits: 2 })}</span>
                            : <span style={{ color: "var(--fg-7)" }}>{quotesLoading ? "…" : "—"}</span>}
                        </td>
                        <td style={{ textAlign: "right" }}>
                        ...
                        <td style={{ textAlign: "right" }}>
                          <span className="font-mono text-sm" style={{ color: "var(--fg-3)" }}>{Number(pos.cantidad).toLocaleString("es-AR", { maximumFractionDigits: 4 })}</span>
                        </td>
                        <td style={{ textAlign: "right" }}>
                          {valorActual != null
                            ? <span className="font-mono text-sm font-semibold" style={{ color: "var(--fg-2)" }}>{pos.moneda === "ARS" ? "$" : "U$S"} {valorActual.toLocaleString("es-AR", { maximumFractionDigits: 0 })}</span>
                            : <span style={{ color: "var(--fg-7)" }}>—</span>}
                        </td>
                        <td style={{ textAlign: "right" }}>
                          {pnl != null ? (
                        ...
                        <td>
                          <div className="flex items-center gap-1 justify-end">
                            <button onClick={() => openEditPos(pos)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-white/5" style={{ color: "var(--fg-6)" }}>
              ...
              <p className="px-5 py-2 text-xs" style={{ color: "var(--fg-7)" }}>Tickers con .BA = mercado argentino (BCBA)</p>
            </div>
          )}
        </div>
      )}
      ...
      {tab === "activos" && (
        dataSeries.length === 0 ? (
          <div className="glass-card p-14 text-center">
            <p className="text-5xl mb-4">🏠</p>
            <h3 className="text-lg font-semibold mb-2" style={{ color: "var(--fg-hi)" }}>Sin activos registrados</h3>
            <p className="text-sm max-w-sm mx-auto mb-6" style={{ color: "var(--fg-5)" }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="glass-card p-6">
              <h2 className="text-base font-semibold mb-4" style={{ color: "var(--fg-hi)" }}>Distribución</h2>
            ...
            <div className="glass-card overflow-hidden">
              <div className="p-5 pb-3"><h2 className="text-base font-semibold" style={{ color: "var(--fg-hi)" }}>Últimas valuaciones</h2></div>
              <table className="data-table">
                ...
                <tbody>
                  {dataSeries.map((v: any, i: number) => {
                    ...
                    return (
                      <tr key={i}>
                        <td><div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: v.color }} /><span className="text-sm font-medium truncate" style={{ color: "var(--fg-5)", maxWidth: 140 }}>{v.name}</span></div></td>
                        <td style={{ textAlign: "right", color: "var(--fg-5)" }} className="font-mono text-sm font-semibold">{v.moneda === "USD" ? `U$S ${Number(v.value).toLocaleString()}` : formatCurrency(v.value, "ARS", true)}</td>
      ...
      {tab === "pasivos" && (
        pasivos.length === 0 ? (
          <div className="glass-card p-14 text-center">
            <p className="text-5xl mb-4">✅</p>
            <h3 className="text-lg font-semibold mb-2" style={{ color: "var(--fg-hi)" }}>Sin pasivos registrados</h3>
            <p className="text-sm max-w-sm mx-auto mb-6" style={{ color: "var(--fg-5)" }}>
                ...
                <div key={p.id} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                  ...
                  <div className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className="text-sm font-semibold" style={{ color: "var(--fg-hi)" }}>{p.nombre}</p>
                      ...
                      {p.fecha_vencimiento && <p className="text-xs" style={{ color: "var(--fg-6)" }}>Vence: {p.fecha_vencimiento}</p>}
                    </div>
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-4 space-y-4" style={{ background: "rgba(0,0,0,0.15)" }}>
                      ...
                      {/* Payment History */}
                      <div>
                        <p className="text-xs font-semibold uppercase mb-2" style={{ color: "var(--fg-5)" }}>Historial de pagos</p>
                        {historyLoading ? (
                          <p className="text-xs" style={{ color: "var(--fg-6)" }}>Cargando...</p>
                        ) : history.length === 0 ? (
                          <p className="text-xs" style={{ color: "var(--fg-7)" }}>Sin pagos registrados aún.</p>
                        ) : (
                            ...
                            {history.map((pg: any) => (
                              <div key={pg.id} className="flex items-center justify-between gap-3 rounded-lg px-3 py-2"
                                ...
                                <div className="min-w-0">
                                  ...
                                  {pg.descripcion && <p className="text-[10px] truncate" style={{ color: "var(--fg-7)" }}>{pg.descripcion}</p>}
                                </div>
                      ...
                      {isUva && p.capital_uva && p.cuota_uva && (
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <p className="text-xs font-semibold uppercase" style={{ color: "var(--fg-5)" }}>
                              Proyección de cuotas
                              <span className="ml-1 font-normal normal-case" style={{ color: "var(--fg-7)" }}>
                          ...
                          {uvaEfectivo ? (
                                ...
                                <table className="w-full text-xs">
                                  <thead>
                                    <tr style={{ background: "rgba(255,255,255,0.04)" }}>
                                      ...
                                      <th className="px-2 py-1.5 text-right" style={{ color: "var(--fg-6)" }}>Int.UVA</th>
                                      <th className="px-2 py-1.5 text-right" style={{ color: "#A5A0FF" }}>Cuota ARS</th>
                                      <th className="px-2 py-1.5 text-right" style={{ color: "var(--fg-6)" }}>UVA est.</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    {proyeccion.map((c) => (
                                      <tr key={c.nCuota} style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }}>
                                        ...
                                        <td className="px-2 py-1.5 text-right font-mono" style={{ color: "var(--fg-6)" }}>{c.interesUva.toFixed(1)}</td>
                                        <td className="px-2 py-1.5 text-right font-mono font-bold" style={{ color: "#A5A0FF" }}>${c.cuotaArs.toLocaleString("es-AR")}</td>
                                        <td className="px-2 py-1.5 text-right font-mono" style={{ color: "var(--fg-5)" }}>${c.uvaVal.toLocaleString("es-AR", { maximumFractionDigits: 0 })}</td>
                                      </tr>
                                    ))}
                                  </tbody>
                                </table>
                            ...
                            <p className="text-xs" style={{ color: "var(--fg-7)" }}>Necesitás el valor UVA actual para proyectar las cuotas.</p>
                          )}
                        </div>
                      )}
                    </div>
      ...
      {tab === "plazos" && (
        plazos.length === 0 ? (
          <div className="glass-card p-12 text-center">
            ...
            <p className="font-semibold mb-1" style={{ color: "var(--fg-hi)" }}>Sin plazos fijos registrados</p>
            <p className="text-sm mb-4" style={{ color: "var(--fg-6)" }}>Agregá tus plazos fijos para hacer un seguimiento</p>
          ...
          <div className="space-y-3">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <p className="font-semibold text-sm" style={{ color: "var(--fg-hi)" }}>{pf.entidad}</p>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <div>
                          <p className="text-[10px] uppercase mb-0.5" style={{ color: "var(--fg-6)" }}>Capital</p>
                          <p className="text-sm font-mono font-semibold" style={{ color: "var(--fg-5)" }}>
                            {pf.moneda === "USD" ? "U$S" : "$"} {Number(pf.monto_inicial).toLocaleString("es-AR")}
                          </p>
                        </div>
                        <div>
                          <p className="text-[10px] uppercase mb-0.5" style={{ color: "var(--fg-6)" }}>Ganancia est.</p>
                        ...
                        <div>
                          <p className="text-[10px] uppercase mb-0.5" style={{ color: "var(--fg-6)" }}>Total al vence</p>
                      ...
                      <p className="text-[10px] mt-1.5" style={{ color: "var(--fg-7)" }}>
                        TNA {pf.tasa_tna}% · {pf.plazo_dias} días · {pf.fecha_inicio} → {pf.fecha_vencimiento}
                      </p>
                    </div>
                    {/* Actions */}
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button onClick={() => openEditPlazo(pf)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-white/5" style={{ color: "var(--fg-6)" }}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                      </button>
            ...
            <div className="glass-card px-5 py-3 flex items-center justify-between">
              <p className="text-sm font-semibold" style={{ color: "var(--fg-5)" }}>Total plazos activos</p>
          ...
          <div className="glass-card w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <button onClick={() => { setShowPlazoModal(false); resetPlazoForm(); }} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/5" style={{ color: "var(--fg-6)" }}>
              ...
              {plMonto && plTna && plPlazo && (
                <div className="rounded-xl p-3 mb-4 space-y-1" style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.20)" }}>
                  ...
                  <div className="flex justify-between text-sm">
                    <span style={{ color: "var(--fg-5)" }}>Ganancia estimada</span>
                  ...
                  <div className="flex justify-between text-sm">
                    <span style={{ color: "var(--fg-5)" }}>Total al vencimiento</span>
              ...
              <form onSubmit={handleSavePlazo} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Banco / Entidad *</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Capital invertido *</label>
                  ...
                  <div>
                    <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>TNA (%) *</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Plazo (días) *</label>
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Fecha de inicio</label>
                ...
                <div className="flex items-center gap-3 py-2">
                  ...
                  <label htmlFor="plazo-renovacion" className="text-sm" style={{ color: "var(--fg-4)" }}>Renovación automática al vencimiento</label>
                </div>
                <div>
                  <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Notas (opcional)</label>
          ...
          <div className="glass-card w-full max-w-md">
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <button onClick={() => { setShowValModal(false); resetValForm(); }} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/5" style={{ color: "var(--fg-6)" }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                </button>
              </div>
              <form onSubmit={handleSaveVal} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Nombre del activo</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Valor</label>
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Fecha de valuación</label>
          ...
          <div className="glass-card w-full max-w-md" style={{ maxHeight: "90vh", overflowY: "auto" }}>
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <button onClick={() => { setShowPosModal(false); resetPosForm(); }} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/5" style={{ color: "var(--fg-6)" }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                </button>
              </div>
              <form onSubmit={handleSavePos} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Ticker *</label>
                    <TickerSearchInput value={posTicker} onChange={setPosTicker} onSelect={(t, n) => { setPosTicker(t); if (!posNombre) setPosNombre(n); }} />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Cantidad *</label>
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Nombre (opcional)</label>
                  <input className="input-field" placeholder="Ej: Apple Inc." value={posNombre} onChange={e => setPosNombre(e.target.value)} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Precio compra</label>
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Broker (opcional)</label>
          ...
          <div className="glass-card w-full max-w-md">
            <div className="p-6">
              <div className="flex items-start justify-between mb-1">
                <div>
                  <h2 className="text-xl font-bold" style={{ color: "var(--fg-hi)" }}>Registrar pago</h2>
                  <p className="text-sm mt-0.5" style={{ color: "var(--fg-6)" }}>{pagoTargetPasivo.nombre}</p>
                </div>
                <button onClick={() => setShowPagoModal(false)} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/5" style={{ color: "var(--fg-6)" }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                </button>
              </div>
              ...
              <div className="rounded-xl p-3 my-4 flex items-center justify-between" style={{ background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.15)" }}>
                <span className="text-xs" style={{ color: "var(--fg-5)" }}>Saldo pendiente actual</span>
              ...
              {pagoTargetPasivo.sistema_amortizacion === "uva" && (
                <div className="rounded-xl p-3 mb-2 space-y-1.5" style={{ background: "rgba(108,99,255,0.08)", border: "1px solid rgba(108,99,255,0.20)" }}>
                  <div className="flex items-center justify-between">
                    ...
                    {uvaEfectivo
                      ? <span className="text-[10px] font-mono" style={{ color: "var(--fg-5)" }}>1 UVA = ${uvaEfectivo.toLocaleString("es-AR", { maximumFractionDigits: 2 })}</span>
                  ...
                  {uvaEfectivo && pagoTargetPasivo.cuota_uva && (
                    <p className="text-xs" style={{ color: "var(--fg-5)" }}>
                      ...
                      <span style={{ color: "var(--fg-6)" }}> ({Number(pagoTargetPasivo.cuota_uva).toLocaleString("es-AR", { maximumFractionDigits: 2 })} UVAs)</span>
                    </p>
                  )}
              ...
              <form onSubmit={handleRegistrarPago} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Monto pagado en ARS *</label>
          ...
          <div className="glass-card w-full max-w-md" style={{ maxHeight: "90vh", overflowY: "auto" }}>
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <button onClick={() => { setShowPasivoModal(false); resetPasivoForm(); }} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/5" style={{ color: "var(--fg-6)" }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                </button>
              </div>
              <form onSubmit={handleSavePasivo} className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase mb-1.5" style={{ color: "var(--fg-5)" }}>Sistema de amortización</label>
                ...
                {pSistema === "uva" ? (
                  <div className="rounded-xl p-4 space-y-3" style={{ background: "rgba(108,99,255,0.08)", border: "1px solid rgba(108,99,255,0.20)" }}>
                    ...
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      ...
                      {uvaLoading ? (
                        <span className="text-xs" style={{ color: "var(--fg-6)" }}>Consultando BCRA…</span>
                      ) : uvaEfectivo ? (
                        <span className="flex items-center gap-2 text-xs font-mono" style={{ color: "var(--fg-4)" }}>
